...
describe('CliRunner Integration Test (Bundled)', () => {
  describe('exports', () => {
    ...
    // 操作: CliRunnerを直接requireで読み込む
    // 期待値: dist/CliRunner.jsから直接インポートできる
    it('should allow direct import from dist/CliRunner.js', () => {
      const CliRunnerDirect = require(path.resolve(__dirname, '../../dist/CliRunner.js')).CliRunner;
      expect(CliRunnerDirect).toBeDefined();
      expect(typeof CliRunnerDirect).toBe('function');
    });
  });
  ...
  describe('instance creation', () => {
    ...
    it('should have expected methods', () => {
      ...
      expect(typeof cli.getStdout).toBe('function');
      expect(typeof cli.getStderr).toBe('function');
    });
  });
  ...
  describe('basic process execution', () => {
    ...
    it('should throw when writing to non-started process', async () => {
      ...
      await expect(newCli.write('test')).rejects.toThrow();
      newCli.dispose();
    });

    // 前提: プロセスが実行中
    // 操作: getStdout()でバッファを参照
    // 期待値: 出力内容が返される
    it('should return stdout buffer after execution', async () => {
      await cli.start({
        command: process.execPath,
        args: ['-v'],
        cwd: process.cwd(),
      ...
      await cli.readStdout().toLines(1000);
      const stdout = cli.getStdout();
      expect(stdout).toMatch(/^v\d+\.\d+\.\d+/);
    });

    // 前提: プロセスが実行中
    // 操作: getStderr()でエラーバッファを参照
    // 期待値: stderrバッファが返される（空の可能性あり）
    it('should return stderr buffer after execution', async () => {
      await cli.start({
        command: process.execPath,
        args: ['-v'],
        cwd: process.cwd(),
  ...
  describe('process disposal and cleanup', () => {
    ...
    it('should clean up process on dispose', async () => {
      ...
      await expect(cli.write('test')).rejects.toThrow();
    });
  });
  ...
  describe('line reading capability', () => {
    ...
    it('should read multiple lines from stdout', async () => {
      ...
      await cli.start({
        command: process.execPath,
        args: ['-e', `console.log("${multiLineOutput.split('\\n').join('");console.log("')}")`],
        cwd: process.cwd(),
      });
